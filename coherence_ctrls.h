...
class MESIBottomCC : public GlobAlloc {
    private:
        ...
        Counter profGETSPrefetch_IS; /*GetS produced due to prefetch requests (address was in I)*/
	Counter profGETSPrefetch_SS; /*GetS produced due to prefetch requests (address was in S)*/
        Counter profGETSPrefetch_ES; /*GetS produced due to prefetch requests (address was in E/M)*/
    ...
    public:
        ...
        void initStats(AggregateStat* parentStat) {
            ...
            profGETSPrefetch_IS.init("pftGETS_IS", "Prefetched request from invalid address");
            profGETSPrefetch_SS.init("pftGETS_SS", "Prefetched request from shared address");
            profGETSPrefetch_ES.init("pftGETS_ES", "Prefetched request from exclusive/modified address");
            ...
            parentStat->append(&profGETSPrefetch_IS);
            parentStat->append(&profGETSPrefetch_SS);
            parentStat->append(&profGETSPrefetch_ES);
        }
...
class MESICC : public CC {
    ...
    public:
        ...
        void initStats(AggregateStat* cacheStat) {
            ...
            //tcc->initStats(cacheStat); //Pratyush
        }
        ...
        uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle, uint64_t* getDoneCycle = nullptr) {
            ...
            if (lineId == -1 || (((req.type == PUTS) || (req.type == PUTX)) && !bcc->isValid(lineId))) { //can only be a non-inclusive wback
                ...
                uint32_t flags = req.flags & ~MemReq::PREFETCH; //always clear PREFETCH, this flag cannot propagate up //Pratyush
                ...
                if (!isPrefetch) { //prefetches only touch bcc; the demand request from the core will pull the line to lower level //Pratyush
