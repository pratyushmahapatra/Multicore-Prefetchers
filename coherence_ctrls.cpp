    ...
    switch (type) {
        ...
        case GETS:
            if (*state == I) {
                ...
                profGETSMiss.inc();
	        if (flags & MemReq::PREFETCH2) {
                    profGETSPrefetch_IS.inc();
	        }
                assert(*state == S || *state == E);
            } else {
                profGETSHit.inc();
	        if (flags & MemReq::PREFETCH2) {
                    profGETSPrefetch_ES.inc();
	        }
	        if (flags & MemReq::PREFETCH2) {
                    profGETSPrefetch_SS.inc();
	        }
            }
            break;
        case GETX:
...
uint64_t MESITopCC::processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint32_t childId, bool haveExclusive,
    ...
    switch (type) {
        ...
        case GETS:
	        ...
	        if (flags & MemReq::PREFETCH2) {
                    printf("Prefetch request when address is in Shared or Excl\n");
	        }
                //Give in S state
